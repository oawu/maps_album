...
class ImageImagickUtility extends ImageBaseUtility {
  ...
  public function __construct ($fileName, $options = array ()) {
    ...
    $this->configs = array (
        ...
        'mime_formats' => array ('image/gif'   => 'gif',
                                 ...
                                 'image/png'   => 'png',
                                 'image/x-png' => 'png'),
        'd4_options' => array ('resizeUp' => true)
      );
  ...
  public function cleanExif () {
    $img = clone $this->image;
    $img->stripImage;

    return $this->_updateImage ($img);
  }
  // return ImageImagickUtility object
  ...
  public static function make_block9 ($files, $save = null, $rawData = true) {
    ...
    for ($i = 0; $i < 9; $i++)
      $newImage->compositeImage (ImageUtility::create ($files[$i], 'ImageImagickUtility')->getImage (),
  ...
  public static function photos ($files, $save = null, $rawData = true) {
    ...
    for ($i = 0; $i < count ($positions); $i++)
      $newImage->compositeImage (ImageUtility::create ($files[$i], 'ImageImagickUtility')->adaptiveResizeQuadrant ($positions[$i]['width'], $positions[$i]['height'])
                                                                                         ->getImage (),
                                 imagick::COMPOSITE_DEFAULT,
                                 $positions[$i]['left'],
                                 $positions[$i]['top']);
