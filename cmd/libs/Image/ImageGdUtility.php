...
class ImageGdUtility extends ImageBaseUtility {
  ...
  public function __construct ($fileName, $options = array ()) {
    ...
    $this->configs = array (
        'allow_formats' => array ('gif', 'jpg', 'png'),
        'mime_formats' => array ('image/gif' => 'gif',
                                 'image/jpeg' => 'jpg',
                                 'image/pjpeg' => 'jpg',
                                 'image/png' => 'png',
                                 'image/x-png' => 'png'),
        'd4_options' => array ('interlace' => null,
                               'resizeUp' => true,
                               'jpegQuality' => 90,
                               'preserveAlpha' => true,
  ...
  public function addBorder ($w, $h, $weight, $color) {
    $newImage = function_exists ('imagecreatetruecolor') ? imagecreatetruecolor ($w, $h) : imagecreate ($w, $h);
    $color = imagecolorallocate ($newImage, $color[0], $color[1], $color[2]);
    for ($i = 0; $i < $weight; $i++) ImageRectangle ($this->image, $i, $i, --$w, --$h, $color);
    return $this;
  }
  // return ImageGdUtility object
  ...
  public static function make_block9 ($files, $save, $interlace = null, $jpegQuality = 100) {
    ...
    for ($i = 0; $i < 9; $i++)
      imagecopymerge ($image,
                      ImageUtility::create ($files[$i], 'ImageGdUtility')->getImage (),
                      $positions[$i]['left'],
                      $positions[$i]['top'],
                      0,
                      0,
  ...
  public static function photos ($files, $save, $interlace = null, $jpegQuality = 100) {
    ...
    $color = array (165, 164, 165);
    for ($i = 0; $i < count ($positions); $i++)
      imagecopymerge ($image,
                      ImageUtility::create ($files[$i], 'ImageGdUtility')->adaptiveResizeQuadrant ($positions[$i]['width'], $positions[$i]['height'], 'c')->addBorder ($positions[$i]['width'], $positions[$i]['height'], 2, $color)->getImage (),
                      $positions[$i]['left'],
                      $positions[$i]['top'],
                      0,
                      0,
